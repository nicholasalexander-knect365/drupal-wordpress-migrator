...
class DB {
	...
	public function rawQuery($sql) {
		...
		if (is_object($result)) {
			...
			throw new Exception("rawQuery result not an object?");
			debug($sql);
		}
	}
	...
	*/
	...
	private function getObjects() {
		...
		if (is_object($this->result)) {
			...
			throw new Exception('DB::getObjects did not have a result object');
		}
		$this->result->close();
	}
	...
	public function getRecords() {
		...
		if (!is_object($this->result)) {
			throw new Exception('getRecords does not have a result object to work with from this query '.$this->sql);
		}
		if ($this->result) {
			$this->getObjects();
			return $this->rows;
